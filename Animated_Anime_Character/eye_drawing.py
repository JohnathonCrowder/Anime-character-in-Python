...
class EyeRenderer:
    ...
    def draw_eye_whites(self, painter, offset_x, offset_y, left_blink, right_blink, emotion_data):
        """Draw eye whites matching original style"""
        # Left eye white
        painter.setPen(QPen(QColor("black"), 2))
        ...
        self.renderer.smooth_r(left_eye_path, 2, 27 - 25*left_blink, -1, 30 - 28*left_blink)
        self.renderer.smooth_r(left_eye_path, -39, 5, -44, 1)
        self.renderer.smooth(left_eye_path, 206 + offset_x, 212 + offset_y, 206 + offset_x, 212 + offset_y)
        
        painter.drawPath(left_eye_path)
        
        # Right eye white
        # Top outline in black, rest in #D1D1D1
        ...
        # Bottom part with no outline
        right_bottom_path = QPainterPath()
        self.renderer.moveto(right_bottom_path, 346 + offset_x, 214 + offset_y)
        self.renderer.smooth_r(right_bottom_path, 3, 18 - 16*right_blink, 6, 23 - 21*right_blink)
        self.renderer.smooth_r(right_bottom_path, 38, 6, 40, 4)
        ...
        self.renderer.smooth_r(full_eye_path, 3, 18 - 16*right_blink, 6, 23 - 21*right_blink)
        self.renderer.smooth_r(full_eye_path, 38, 6, 40, 4)
    ...
    def draw_irises(self, painter, offset_x, offset_y, left_blink, right_blink, emotion_data, winking):
        """Draw irises matching original style"""
    ...
    def draw_eye_lines(self, painter, offset_x, offset_y, left_blink, right_blink, winking):
        ...
        self.renderer.moveto(curve_path, 225 + offset_x, 215 + offset_y)
        self.renderer.curveto_r(curve_path, 10, 28, 22, 16, 24, 6)
        painter.drawPath(curve_path)
        
        right_curve_path = QPainterPath()
        self.renderer.moveto(right_curve_path, 365 + offset_x, 219 + offset_y)
        self.renderer.curveto_r(right_curve_path, 4, 14, 18, 24, 22, -3)
        ...
        if right_blink < 0.4 and not winking:
            ...
            for x1, y1, x2, y2 in right_lash_lines:
                painter.drawLine(QPointF(x1 + offset_x, y1 + offset_y), 
                               QPointF(x2 + offset_x, y2 + offset_y))